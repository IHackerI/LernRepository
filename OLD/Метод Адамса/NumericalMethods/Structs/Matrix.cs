namespace NumericalMethods
{
    ...
    public class Matrix 
    {
        ...
        public (int Row, double Value) MaxInColumn(int column)
        {
            var result = (Row : -1, Value : double.MinValue);

            for (var index = 0; index < this.Rows; index++)
                if (result.Value < this[index, column])
                    result = (index, this[index, column]);

            return result;
        }

        public (int Column, double Value) MaxInRow(int row)
        {
            var result = (Row: -1, Value: double.MinValue);

            for (var index = 0; index < this.Columns; index++)
                if (result.Value < this[row, index])
                    result = (index, this[row, index]);
