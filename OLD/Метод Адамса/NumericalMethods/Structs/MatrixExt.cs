namespace NumericalMethods.Structs
{
    ...
    public static class MatrixExt
    {
        ...
        public static (Matrix R, Matrix T) Orthogonalization(this Matrix m)
        {
            var result = (R: new Matrix(m.Rows, m.Columns), T: new Matrix(m.Rows, 1.0));
        ...
        public static void Transform(this Matrix m, Vector b)
        {
            ...
            for (var row = 0; row < m.Rows; ++row)
            {
                ...
                if (tmp.Row == row) continue;

                m.SwapRows(tmp.Row, row);
                b.Swap(tmp.Row, row);
            }
